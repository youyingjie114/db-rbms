...
// sql语句查询，判断订单中的pur是否存在
$sql = "SELECT * FROM purchases WHERE pur = '$pur'";
$res = mysqli_query($conn, $sql);
$pur_flag = 0;
if (mysqli_num_rows($res))
    $pur_flag = 1;

// 如果订单中的所有参数都是合法的，就调用存储过程add_purchase实现订单的提交
if ($pur_flag == 0 && $cid_flag && $eid_flag && $pid_flag && $buy_flag) {
    $sql = "CALL add_purchase('$_POST[pur]', '$_POST[cid]', '$_POST[eid]', '$_POST[pid]', $_POST[qty]);";
    $res = mysqli_query($conn, $sql);
}
